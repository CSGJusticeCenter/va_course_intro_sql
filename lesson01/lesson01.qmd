...
- The `residents` table records basic details about individuals, including their assigned facility.
- The `facilities` table stores information about each facility.
- The `facility_id` column in the `residents` table is a **foreign key** that links each person to a facility in the `facilities` table.
...
```{.sql .interactive .state_doc}
SELECT resident_id, last_name, dob
FROM residents;
```
